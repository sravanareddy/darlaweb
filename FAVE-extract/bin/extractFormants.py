...
def anae(v, formants, times):
    ...
    if v == 'AE':
        ...
        print "FATAL!!", F1, times
    measurementPoint = times[i]

    return measurementPoint


def calculateMeans(measurements, stopwords = []):
...
def getTimeOfF1Maximum(formants, times, beg_cutoff, end_cutoff):
    ...
    if trimmedFormants==[]:   #problem if this is empty, so just return 1/2 point
...
def normalize(measurements, m_means):
    ...
    max_f2 = None
    for m in measurements:
        try:  #changed to not scale
            m.norm_f1 = lobanov(m.f1, grand_means[0], grand_stdvs[0])
            if not min_f1 or m.norm_f1<min_f1:
                min_f1 = m.norm_f1
            if not max_f1 or m.norm_f1>max_f1:
                max_f1 = m.norm_f1
        except TypeError:
            m.norm_f1 = ''
        try:
            m.norm_f2 = lobanov(m.f2, grand_means[1], grand_stdvs[1])
            if not min_f2 or m.norm_f2<min_f2:
                min_f2 = m.norm_f2
            if not max_f2 or m.norm_f2>max_f2:
                max_f2 = m.norm_f2
        except TypeError:
            m.norm_f2 = ''
        try:
            m.norm_f3 = lobanov(m.f3, grand_means[2], grand_stdvs[2])
        except TypeError:
            m.norm_f3 = ''
    ...
    #scale according to formula in http://ncslaap.lib.ncsu.edu/tools/norm/about_norm.php
    for m in measurements:
        try:
            m.scaled_f1 = 250 + 500*(m.norm_f1-min_f1)/(max_f1-min_f1)
        except TypeError:
            m.scaled_f1 = ''
        try:
            m.scaled_f2 = 850 + 1400*(m.norm_f2-min_f2)/(max_f2-min_f2)
        except TypeError:
            m.scaled_f2 = ''
...
def outputMeasurements(outputFormat, measurements, m_means, speaker, outputFile, outputHeader, tracks):
    ...
    if outputFormat in ['txt', 'text', 'both']:
        fw = open(outputFile + "_formants.csv", 'w')
        ...
        if outputHeader:
            ...
            fw.write(','.join(['vowel', 'stress', 'pre_word', 'word', 'fol_word',
                                'F1', 'F2', 'F3', 'F1_LobanovNormed_unscaled', 'F2_LobanovNormed_unscaled',
                                'B1', 'B2', 'B3', 't', 'beg', 'end', 'dur',
                                'plt_vclass', 'plt_manner', 'plt_place',
                                'plt_voice', 'plt_preseg', 'plt_folseq',
                                'pre_seg', 'fol_seg', 'context',
        ...
        for vm in measurements:
            ...
            fw.write(',')
            fw.write(','.join(map(str, [vm.f1, vm.f2, vm.f3, vm.norm_f1, vm.norm_f2])))   #formants and normalized values
            fw.write(',')
            fw.write(','.join(map(str, [vm.b1, vm.b2, vm.b3])))  #bandwidths
            fw.write(',')
        ...
        print "Vowel measurements output in .txt format to the file %s" % (outputFile + "_formants.csv")
...
def readSpeakerFile(speakerFile):
    ...
    speaker_parser = argparse.ArgumentParser(description="parses a .speaker file", fromfile_prefix_chars='+')
...
def extractFormants(wavInput, tgInput, output, opts, SPATH='', PPATH=''):
    ...
    for (wavFile, tgFile, outputFile) in zip(wavFiles, tgFiles, outputFiles):
        ...
        if opts.speaker:
            ...
            speaker = readSpeakerFile("default.speaker")
            """
        ...
        m_means = calculateMeans(measurements, opts.stopWords)
